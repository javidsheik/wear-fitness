...
#include "URL.h"
@interface WaterTrackViewController ()

@end

@implementation WaterTrackViewController
...
- (void)viewDidLoad
{
    ...
    _unitlist=[[NSArray alloc] initWithObjects:@"Litre",@"ML.",@"Oz",@"Glass",@"Bottle",nil];
...
-(void)setcolorsetting
{
    ...
    [_watterunit setTitle:@"Litre" forState:UIControlStateNormal];
...
-(void)openpicker
{
    ...
    [_pic setFrame:CGRectMake(20,280,160,20)];
...
//following ifunction send user data to server this function invoked on save action
-(void)saveuserdata
{
    ...
    if (data)
    {
        ...
        NSDictionary *postdata=@{@"user_id":userid,@"track_type":@"Water",
        ...
        [request POST:WaterTrack parameters:postdata success:^(AFHTTPRequestOperation *operation, id responseObject)
         {
