        ...
        
        {
            ...
            if (_showHorizontalLabelsWithInteger)
            {
                ...
                for (int i = 0; i <= horizontalLabelsCount; i ++)
                {
                    ...
                    horizontalLine.backgroundColor =[self colorFromHexString:@"#eeedeb"];  //ELightGrey;
            ...
            else
            {
                ...
                int heightGap = self.frame.size.height / 10.0;
                int valueGap = _fullValueOfTheGraph / 10.0;
                for (int i = 0; i < 11; i++)
                {
                    
                    //add and remove line form here -added by dilip
                    UIView *horizontalLine = [[UIView alloc] initWithFrame:CGRectMake(0, heightGap * i, self.frame.size.width, HORIZONTAL_LINE_HEIGHT)];
                    horizontalLine.backgroundColor =[self colorFromHexString:@"#eeedeb"];
                    [self addSubview:horizontalLine];
                    
                    EColumnChartLabel *eColumnChartLabel = [[EColumnChartLabel alloc] initWithFrame:CGRectMake(-1 * Y_COORDINATE_LABEL_WIDTH, -heightGap / 2.0 + heightGap * i, Y_COORDINATE_LABEL_WIDTH, heightGap)];
                    [eColumnChartLabel setTextAlignment:NSTextAlignmentCenter];
                    eColumnChartLabel.text = [[NSString stringWithFormat:@"%d ", valueGap * (10 - i)] stringByAppendingString:[_dataSource highestValueEColumnChart:self].unit];;
...
- (void)initData
{
    ...
    _minColumnColor =[self colorFromHexString:[[NSUserDefaults standardUserDefaults] objectForKey:@"color"]];          //EMinValueColor;
    _maxColumnColor =[self colorFromHexString:[[NSUserDefaults standardUserDefaults] objectForKey:@"color"]];        //EMaxValueColor;
...
- (void)reloadData
{
    ...
    for (int i = 0; i < totalColumnsRequired; i++)
    {
        ...
        if (nil == eColumn)
        {
            ...
            if (eColumnDataModel.value!=0 && eColumnDataModel.value!=1)
            {
        ...
        if (nil == eColumnChartLabel)
        {
            ...
            //this is label code added and removed from here
            //[self addSubview:eColumnChartLabel];
    ...
    //bottom line cod eis here u can manage coloe animation time width here
    [UIView animateWithDuration:0.8 delay:0 options:UIViewAnimationOptionCurveEaseIn animations:^
     {
         ...
         bottomLine.backgroundColor = [self colorFromHexString:@"#747272"];
         bottomLine.layer.cornerRadius = 2.0;
         [self addSubview:bottomLine];
         [bottomLine setFrame:CGRectMake(0, self.frame.size.height, self.frame.size.width-2, BOTTOM_LINE_HEIGHT)];
